...
DISABLE_FLEX_ATTENTION = os.environ.get("CAUSAL_DISABLE_FLEX_ATTENTION") == "1"

try:
    if DISABLE_FLEX_ATTENTION:
        raise ModuleNotFoundError(
            "flex attention disabled by CAUSAL_DISABLE_FLEX_ATTENTION"
        )
    ...
    def flex_attention(*args, **kwargs):
        raise RuntimeError(
            "torch.nn.attention.flex_attention is unavailable in this PyTorch build."
        )


def _rank0_debug(msg: str):
    if os.environ.get("CAUSAL_DEBUG_STAGES") == "1" and (
        not dist.is_initialized() or dist.get_rank() == 0
    ):
        print(msg, flush=True)


if DISABLE_FLEX_ATTENTION and (not dist.is_initialized() or dist.get_rank() == 0):
    print(
        "CAUSAL_DISABLE_FLEX_ATTENTION=1 -> using segmented attention fallback",
        flush=True,
    )
...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            attention_block_size = frame_seqlen * self.num_frame_per_block

            def run_segmented_flash_attention(query, key, value, clean_tokens, is_tf):
                outputs = []
                total_tokens = query.shape[1]

                for block_start in range(0, clean_tokens, attention_block_size):
                    block_end = min(block_start + attention_block_size, clean_tokens)
                    outputs.append(
                        attention(
                ...
                if is_tf:
                    for block_idx, block_start in enumerate(
                        range(clean_tokens, total_tokens, attention_block_size)
                    ):
                        block_end = min(
                            block_start + attention_block_size, total_tokens
                        )
                        key_segments = []
                        value_segments = []
                        clean_prefix_end = min(
                            block_idx * attention_block_size, clean_tokens
                        )
                        if clean_prefix_end > 0:
                            key_segments.append(key[:, :clean_prefix_end])
                            value_segments.append(value[:, :clean_prefix_end])
                        key_segments.append(key[:, block_start:block_end])
                        value_segments.append(value[:, block_start:block_end])
                        outputs.append(
                            attention(
                                query[:, block_start:block_end],
                                torch.cat(key_segments, dim=1),
                                torch.cat(value_segments, dim=1),
                                causal=False,
                            )
                        )
            ...
            if is_tf:
                ...
                if HAS_FLEX_ATTENTION:
                    ...
                    x = run_segmented_flash_attention(
                        roped_query,
                        roped_key,
                        v,
