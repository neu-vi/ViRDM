...
def _optional_step_tensor(values):
    steps = list(values or [])
    return torch.tensor(steps, dtype=torch.long) if steps else None
...
class CausalInferencePipeline(torch.nn.Module):
    def __init__(self, args, device, generator=None, text_encoder=None, vae=None):
        ...
        self.first_frame_denoising_step_list = _optional_step_tensor(
            getattr(args, "first_frame_denoising_step_list", [])
        )
        if args.warp_denoising_step:
            ...
            if self.first_frame_denoising_step_list is not None:
                self.first_frame_denoising_step_list = timesteps[
                    1000 - self.first_frame_denoising_step_list
                ]
    ...
    def inference(
        ...
        if return_latents:
            ...
            return video if return_video else output
